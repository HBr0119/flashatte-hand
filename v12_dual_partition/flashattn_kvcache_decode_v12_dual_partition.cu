/*
 * v12 dual-partition hybrid: v11/v10 BSM direct path for KV <= 2048,
 * best-style lightweight partition path for KV > 2048.
...
struct alignas(16) v12_float8_t {
    float value[VEC_SIZE];
};

union alignas(16) v12_packed_bf16x8_t {
    int4 bits;
    uint32_t words[4];
};

union v12_packed_bf16x2_t {
    uint32_t bits;
    __nv_bfloat162 value;
};

__device__ __forceinline__ v12_float8_t v12_load_bf16x8(
    const __nv_bfloat16* ptr) {
    v12_packed_bf16x8_t packed;
    packed.bits = *reinterpret_cast<const int4*>(ptr);
    v12_packed_bf16x2_t p0, p1, p2, p3;
    ...
    v12_float8_t result;
    result.value[0] = f0.x; result.value[1] = f0.y;
    result.value[2] = f1.x; result.value[3] = f1.y;
    result.value[4] = f2.x; result.value[5] = f2.y;
    result.value[6] = f3.x; result.value[7] = f3.y;
    return result;
}

__device__ __forceinline__ float v12_dot8(
    const v12_float8_t& a, const v12_float8_t& b) {
    float result = 0.f;
    #pragma unroll
    for (int i = 0; i < VEC_SIZE; ++i) result = fmaf(a.value[i], b.value[i], result);
    return result;
}

__device__ __forceinline__ void v12_zero8(v12_float8_t& value) {
    #pragma unroll
    for (int i = 0; i < VEC_SIZE; ++i) value.value[i] = 0.f;
}

__device__ __forceinline__ void v12_scale8(v12_float8_t& value, float scale) {
    #pragma unroll
    for (int i = 0; i < VEC_SIZE; ++i) value.value[i] *= scale;
}

__device__ __forceinline__ void v12_fma8(
    v12_float8_t& accumulator, const v12_float8_t& value, float weight) {
    #pragma unroll
    for (int i = 0; i < VEC_SIZE; ++i) {
        accumulator.value[i] = fmaf(weight, value.value[i], accumulator.value[i]);
    }
}

__device__ __forceinline__ void v12_store_float8(
    float* ptr, const v12_float8_t& value) {
    *reinterpret_cast<float4*>(ptr) = make_float4(
        value.value[0], value.value[1], value.value[2], value.value[3]);
    *reinterpret_cast<float4*>(ptr + 4) = make_float4(
        value.value[4], value.value[5], value.value[6], value.value[7]);
}

__device__ __forceinline__ v12_float8_t v12_load_float8(const float* ptr) {
    const float4 lo = *reinterpret_cast<const float4*>(ptr);
    const float4 hi = *reinterpret_cast<const float4*>(ptr + 4);
    v12_float8_t result;
    result.value[0] = lo.x; result.value[1] = lo.y;
    result.value[2] = lo.z; result.value[3] = lo.w;
    result.value[4] = hi.x; result.value[5] = hi.y;
    result.value[6] = hi.z; result.value[7] = hi.w;
    return result;
}
...
template <int GROUP_SIZE, int BDZ>
__global__ __launch_bounds__(BDX * GROUP_SIZE * BDZ)
void paged_decode_partition_kernel_v12_best(
    const __nv_bfloat16* __restrict__ q,
    const __nv_bfloat16* __restrict__ k_cache_paged,
    const __nv_bfloat16* __restrict__ v_cache_paged,
    float* __restrict__ partial_o,
    ...
    int num_kv_heads,
    int num_splits) {
    static_assert(BDX * GROUP_SIZE * BDZ == 256, "v12 partition expects 256 threads");
    ...
    __shared__ __align__(16) float merge_o[BDZ * GROUP_SIZE * HEAD_DIM];
    __shared__ float merge_lse[BDZ * GROUP_SIZE];
    __shared__ float merge_weight[BDZ * GROUP_SIZE];
    __shared__ float merge_denominator[GROUP_SIZE];
    __shared__ float merge_max_lse[GROUP_SIZE];

    constexpr int THREADS = BDX * GROUP_SIZE * BDZ;
    constexpr int PACKS_PER_TOKEN = HEAD_DIM / VEC_SIZE;
    constexpr float SCALE_LOG2 =
        0.08838834764831845f * 1.4426950408889634f;
    ...
    const int linear_tid = tx + BDX * (ty + GROUP_SIZE * tz);
    ...
    const int last_page = static_cast<int>(
        static_cast<int64_t>(valid_pages) * (split_idx + 1) / num_splits);
    const int64_t partial_idx = bh * num_splits + split_idx;
    const int64_t partial_base = partial_idx * HEAD_DIM;
    const int dim_base = tx * VEC_SIZE;

    if (first_page >= last_page) {
        if (tz == 0) {
            v12_float8_t zero;
            v12_zero8(zero);
            v12_store_float8(partial_o + partial_base + dim_base, zero);
            if (tx == 0) partial_lse[partial_idx] = -CUDART_INF_F;
        }
        return;
    }

    const v12_float8_t q_vec = v12_load_bf16x8(q + bh * HEAD_DIM + dim_base);
    v12_float8_t accumulator;
    v12_zero8(accumulator);
    float running_max = -CUDART_INF_F;
    float running_sum = 0.f;
    ...
    for (int logical_page = first_page; logical_page < last_page; ++logical_page) {
        const int token_begin = logical_page * PAGE_SIZE;
        int page_tokens = seqlen - token_begin;
        if (page_tokens > PAGE_SIZE) page_tokens = PAGE_SIZE;
        const int physical_page = __ldg(block_row + logical_page);
        const int64_t page_base =
            static_cast<int64_t>(physical_page) * page_stride + head_offset;
        const int pack_count = page_tokens * PACKS_PER_TOKEN;
        ...
        for (int pack = linear_tid; pack < pack_count; pack += THREADS) {
            ...
            const int dim = (pack - token * PACKS_PER_TOKEN) * VEC_SIZE;
            const int64_t src =
                page_base + static_cast<int64_t>(token) * token_stride + dim;
            const int dst = token * HEAD_DIM + dim;
            *reinterpret_cast<int4*>(shared_k + dst) =
                *reinterpret_cast<const int4*>(k_cache_paged + src);
            *reinterpret_cast<int4*>(shared_v + dst) =
                *reinterpret_cast<const int4*>(v_cache_paged + src);
        }
        __syncthreads();

        #pragma unroll 1
        for (int token = tz; token < page_tokens; token += BDZ) {
            const int smem_base = token * HEAD_DIM + dim_base;
            const v12_float8_t k_vec = v12_load_bf16x8(shared_k + smem_base);
            const float score =
                subgroup_sum_16_v11(v12_dot8(q_vec, k_vec)) * SCALE_LOG2;
            const float new_max = fmaxf(running_max, score);
            const float alpha = running_sum > 0.f ? exp2f(running_max - new_max) : 0.f;
            const float probability = exp2f(score - new_max);
            v12_scale8(accumulator, alpha);
            const v12_float8_t v_vec = v12_load_bf16x8(shared_v + smem_base);
            v12_fma8(accumulator, v_vec, probability);
            running_sum = fmaf(alpha, running_sum, probability);
            running_max = new_max;
        }
        __syncthreads();
    }
    ...
    const int64_t merge_base =
        static_cast<int64_t>(state_idx) * HEAD_DIM + dim_base;
    const float local_inv = running_sum > 0.f ? 1.f / running_sum : 0.f;
    v12_scale8(accumulator, local_inv);
    v12_store_float8(merge_o + merge_base, accumulator);
    if (tx == 0) {
        merge_lse[state_idx] = running_sum > 0.f
            ? running_max + log2f(running_sum) : -CUDART_INF_F;
    }
    __syncthreads();
    ...
    if (tz == 0 && tx == 0) {
        ...
        #pragma unroll
        for (int z = 0; z < BDZ; ++z) {
            max_lse = fmaxf(max_lse, merge_lse[z * GROUP_SIZE + ty]);
        }
        float denominator = 0.f;
        #pragma unroll
        for (int z = 0; z < BDZ; ++z) {
            const int idx = z * GROUP_SIZE + ty;
            const float lse = merge_lse[idx];
            const float weight = lse == -CUDART_INF_F ? 0.f : exp2f(lse - max_lse);
            merge_weight[idx] = weight;
            denominator += weight;
        }
    ...
    if (tz == 0) {
        ...
        for (int z = 0; z < BDZ; ++z) {
            const int idx = z * GROUP_SIZE + ty;
            const v12_float8_t source = v12_load_float8(
                merge_o + static_cast<int64_t>(idx) * HEAD_DIM + dim_base);
            v12_fma8(merged, source, merge_weight[idx]);
        }
        const float denominator = merge_denominator[ty];
        v12_scale8(merged, denominator > 0.f ? 1.f / denominator : 0.f);
        v12_store_float8(partial_o + partial_base + dim_base, merged);
        if (tx == 0) {
            partial_lse[partial_idx] = denominator > 0.f
                ? merge_max_lse[ty] + log2f(denominator) : -CUDART_INF_F;
        }
    }
}

__global__ void combine_lse_kernel_v12(
    ...
    int64_t causal)
{
    ...
    if (group_size == 8) {
        paged_decode_partition_kernel_v12_best<8, 2><<<part_grid, block>>>(
            q, k_cache_paged, v_cache_paged,
            g_partial_o, g_partial_lse,
            cache_seqlens, block_table,
            blocks_per_batch, nkh, num_splits);
    } else {
        paged_decode_partition_kernel_v12_best<4, 4><<<part_grid, block>>>(
            q, k_cache_paged, v_cache_paged,
            g_partial_o, g_partial_lse,
            cache_seqlens, block_table,
            blocks_per_batch, nkh, num_splits);
    ...
    combine_lse_kernel_v12<<<total_q_heads, HEAD_DIM>>>(
