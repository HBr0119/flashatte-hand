...
// --- MACA C500 xcore MMA fragment types ---
// <mctlass/array.h> guards these under __MACA_ARCH__ which cu-bridge
// does not define, so we declare them locally.
...
#define MMA_N_TILES    (HEAD_DIM / MMA_TILE)   // 8
...
__device__ __forceinline__ void zero_mma_accumulator(
    mma_float_fragment_t& f) {
#pragma unroll
    for (int i = 0; i < 4; ++i) f.element[i] = 0.0f;
}
...
void paged_decode_gqa_page_softmax_kernel(
    ...
    for (int page = 0; page < valid_pages; ++page) {
        ...
        if (page_tokens == PAGE_SIZE) {
            // --- MMA fast path for full pages (16 tokens) ---
            // Pre-load Q fragments. Each thread handles 4 Q dims per K-tile,
            // spread across 16 threads to cover all 16 dims of the tile.
            // 4 virtual query rows all load from the same physical query head.
...
#pragma unroll
                for (int i = 0; i < 4; ++i) {
                    const int linear = sublane * 4 + i;
                    const int dim =
                        kt * MMA_TILE + (linear & 15);
                    q_frag[kt].element[i] =
                        q[q_base + dim];
                }
            }

            mma_float_fragment_t score_acc;
            zero_mma_accumulator(score_acc);

            // MMA: Q[query_head][128] · K[16tokens][128] → score[16]
            // 8 K-tiles of 16 dims each, accumulate into score_acc.
#pragma unroll
            for (int kt = 0; kt < MMA_K_TILES; ++kt) {
...
#pragma unroll
                for (int i = 0; i < 4; ++i) {
                    ...
                    const int dim =
                        kt * MMA_TILE + k_in_tile;
                    k_frag.element[i] =
                        shared_k[token * HEAD_DIM + dim];
                }
            ...
            // Write scores into shared memory for online softmax.
            // Each thread stores 4 token scores (sublane*4..sublane*4+3).
#pragma unroll
            for (int i = 0; i < 4; ++i) {
                const int linear = sublane * 4 + i;
                const int token = linear & 15;
                score_row[token] =
                    score_acc.element[i] * kScale;
            }
        } else {
            // --- Scalar fallback for partial pages (< 16 tokens) ---
#pragma unroll 1
            for (int token = 0; token < page_tokens; ++token) {
...
extern "C" void run_kernel(
    ...
    if (use_page_softmax_direct_gqa) {
        ...
        if (num_heads_k == 4) {
            paged_decode_gqa_page_softmax_kernel<4>
                <<<total_kv_heads, 128>>>(
                    q,
                    k_cache_paged,
                    v_cache_paged,
            ...
            paged_decode_gqa_page_softmax_kernel<8>
                <<<total_kv_heads, 64>>>(
                    q,
                    k_cache_paged,
                    v_cache_paged,
