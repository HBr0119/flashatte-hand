...
void paged_decode_gqa_mma_kernel(
    ...
    // -------- Q-load zero for masked heads ----------
    const __nv_bfloat16 zero_bf16 = __float2bfloat16(0.0f);

    /* Per-thread accumulator and shared softmax state */
            ...
            mma_bf16_fragment_t q_frag;
            {
                ...
                if (a_row < kGroup) {
                    ...
                    const int64_t q_base =
                        ...
                        HEAD_DIM;
                    q_frag.element[0] =
                        q[q_base + k_base_q + 0 + a_k_off];
                    q_frag.element[1] =
                        q[q_base + k_base_q + 1 + a_k_off];
                    q_frag.element[2] =
                        q[q_base + k_base_q + 2 + a_k_off];
                    q_frag.element[3] =
                        q[q_base + k_base_q + 3 + a_k_off];
                } else {
