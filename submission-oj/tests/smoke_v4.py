#!/usr/bin/env python3
"""Quick smoke test for v4 MMA P×V fix."""
import os
...
# Use the same compilation path as the eval script
BINDING_CPP = """
...
# Test the kv=141 case (MMA GQA path)
NUM_KV_HEADS = 4
NUM_HEADS = 32
HEAD_DIM = 128
PAGE_SIZE = 16
...
num_blocks = batch_size * blocks_per_batch  # 72
...
# contest-style seqlens: first is full, second is 1, rest vary
vals = [seqlen_k, 1]
...
print(f"v4 kv={seqlen_k}: NaN={has_nan}, Inf={has_inf}")
print(f"Output stats: min={out_np.min():.6f}, max={out_np.max():.6f}, mean={out_np.mean():.6f}")

# Also check: compute reference using v2 for comparison
# We just check for NaN for now
if has_nan:
    print("FAIL: NaN detected in v4 output!")
    sys.exit(1)
else:
    print("PASS: No NaN in v4 output!")
