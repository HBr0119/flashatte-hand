#!/usr/bin/env python3
"""Quick smoke test for v4 with O1 optimization."""
import os
...
build_dir = "/tmp/smoke_v4_o1_build"
os.makedirs(build_dir, exist_ok=True)
with open(os.path.join(build_dir, "binding.cpp"), "w") as f:
    f.write(BINDING_CPP)

print("Compiling v4 with -O1...")
mod = load(
    name="flashattn_smoke_v4_O1",
    sources=[
...
print(f"v4 O1 kv={seqlen_k}: NaN={has_nan}, Inf={has_inf}")
print(f"Output stats: min={out_np.min():.6f}, max={out_np.max():.6f}, mean={out_np.mean():.6f}")

if has_nan:
    print("FAIL: NaN detected with -O1!")
else:
    print("PASS: No NaN with -O1!")
