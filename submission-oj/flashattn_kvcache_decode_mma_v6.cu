...
/* 8-lane reduction for softmax per-head (8 threads/head in contiguous lanes) */
__device__ __forceinline__ float subgroup_max_8(float value, unsigned mask) {
#pragma unroll
...
__device__ __forceinline__ float subgroup_sum_8(float value, unsigned mask) {
#pragma unroll
    for (int offset = 4; offset > 0; offset >>= 1) {
...
void paged_decode_gqa_mma_kernel(
    ...
    float output_acc[kDimsPerThread];
...
#pragma unroll
    ...
    for (int page = 0; page < valid_pages; ++page) {
        ...
        /* ---- CP-async load K and V (IDENTICAL to baseline) ---- */
        for (int pack = tid;
             pack < pack_count;
             pack += MMA_THREADS) {
            const int token = pack / kPacksPerToken;
        ...
        /* Q fragment reloaded per K-tile to reduce register pressure */
        zero_mma_accumulator(score_fragment);

        const int query_head = kv_head * kGroup + a_row;
        const int64_t q_base =
            (static_cast<int64_t>(b) * kNumHeads + query_head) * HEAD_DIM;

#pragma unroll
        for (int kt = 0; kt < MMA_K_TILES; ++kt) {
            /* Load Q fragment for this K-tile */
            mma_bf16_fragment_t q_frag;
            {
                const int k_base_q = kt * MMA_TILE + warp_id * 8;
                if (a_row < kGroup) {
                    q_frag.element[0] = q[q_base + k_base_q + 0 + a_k_off];
                    q_frag.element[1] = q[q_base + k_base_q + 1 + a_k_off];
                    q_frag.element[2] = q[q_base + k_base_q + 2 + a_k_off];
                    q_frag.element[3] = q[q_base + k_base_q + 3 + a_k_off];
                } else {
                    q_frag.element[0] = zero_bf16;
                    q_frag.element[1] = zero_bf16;
                    q_frag.element[2] = zero_bf16;
                    q_frag.element[3] = zero_bf16;
                }
            }
...
#pragma unroll
        ...
        /* ---- Online softmax (warp-parallel: all threads per head) ---- */
        {
            const int head = head_in_group;

            /* Each thread handles PAGE_SIZE/kThreadsPerHead tokens (1 or 2) */
            float local_max = -CUDART_INF_F;
#pragma unroll
            for (int t = thread_in_head; t < PAGE_SIZE; t += kThreadsPerHead) {
                if (t < page_tokens) {
                    local_max = fmaxf(local_max, shared_scores[head][t]);
                }
            }

            /* Warp shuffle reduction across kThreadsPerHead lanes */
            const unsigned mask = (kThreadsPerHead == 8)
                ? (0xFFu << ((head & 3) * 8))
                : (0xFFFFu << ((head & 1) * 16));

            float page_max = local_max;
#pragma unroll
            for (int offset = kThreadsPerHead / 2; offset > 0; offset >>= 1) {
                page_max = fmaxf(page_max,
                    __shfl_xor_sync(mask, page_max, offset));
            }

            /* Thread 0 per head computes merged_max and alpha */
            float merged_max = 0.0f, alpha = 0.0f;
            if (thread_in_head == 0) {
                merged_max = fmaxf(shared_running_max[head], page_max);
                alpha = shared_running_sum[head] > 0.0f
                    ? expf(shared_running_max[head] - merged_max) : 0.0f;
            }
            merged_max = __shfl_sync(mask, merged_max, 0);
            alpha = __shfl_sync(mask, alpha, 0);

            /* Each thread computes exp and local sum */
            float local_sum = 0.0f;
#pragma unroll
            for (int t = thread_in_head; t < PAGE_SIZE; t += kThreadsPerHead) {
                float prob = 0.0f;
                if (t < page_tokens) {
                    prob = expf(shared_scores[head][t] - merged_max);
                }
            ...
            float page_sum = local_sum;
#pragma unroll
            for (int offset = kThreadsPerHead / 2; offset > 0; offset >>= 1) {
                page_sum += __shfl_xor_sync(mask, page_sum, offset);
            }

            /* Thread 0 per head writes shared state */
            if (thread_in_head == 0) {
                shared_page_alpha[head] = alpha;
                shared_merged_max[head] = merged_max;
                shared_new_sum[head] =
        ...
        if (head_in_group < kGroup) {
            const float alpha =
                shared_page_alpha[head_in_group];
...
#pragma unroll
        for (int d = 0; d < kDimsPerThread; d += 2) {
            output[output_base + dim_base + d] =
                __float2bfloat16(
                    output_acc[d] * inv_denom);
            output[output_base + dim_base + d + 1] =
                __float2bfloat16(
                    output_acc[d + 1] * inv_denom);
        }
    }
}
